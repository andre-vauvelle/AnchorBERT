...
class BinomialMixtureModel:
    ...
    def __init__(self, n_components, code_idx=0, tolerance=1e-5, max_step=1000, verbose=False):
        """
        ...
        code_idx: Which index to aggregate over to find n from vector of values
        tolerance: Float
    ...
    def calc_Posterior(self, N_ls, n_ls):
        ...
        '''
        pi_list = self.pi_list
        theta_list = self.theta_list + EPS
        # K = self.K
        S = len(N_ls)
        ...
        unity_minus_theta_ratio = torch.div((1e0 - theta_list).reshape(1, self.K),
    ...
    def aggregate(self, X):
        """
        Aggregates variable length lists of codes into pairs of N - total length and n - number of codes of interest
        :param X:  list of np_arrays of shape d and length S. to be aggregated into
        :return:
             N_ls: tensor of shape [S]
                   input data
             n_ls: tensor of shape [S]
    ...
    def fit(self, X):
        '''
        Modified to use X instead which is sequence of multi hot vectors. Index of column of interest is set on init.
        ...
        Input
        -----
        X: list of np_arrays of shape d and length S. to be aggregated into
             N_ls: tensor of shape [S]
                   input data
             n_ls: tensor of shape [S]
                   input data
        ...
        while cond_step & cond_params:
            ...
            print("log_likelihood: {}".format(log_likelihood_new))
    ...
    def predict(self, X):
        ...
        '''
        N_ls, n_ls = self.aggregate(X)
        Posterior = self.calc_Posterior(N_ls, n_ls).reshape(-1, self.K)
        return Posterior
