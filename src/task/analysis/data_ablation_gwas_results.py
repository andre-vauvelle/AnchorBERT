...
def plink_trails_results(filter_term='411.2_ca0_co0',
...
if __name__ == '__main__':
    ...
    for code, control_flag in code_control:
        try:
            ...
            check_counts = df_all.groupby(['phenotype', 'ablation', 'trial']).log_pval.count()
            ...
            df_results = merge_on_ld_and_id(df_all_ld, catalog_ld)
            ...
            total_snps = df_results_sub.rsID.unique().shape[0]

            counts_p_trial = df_results_sub.groupby(['phenotype', 'ablation', 'trial']).apply(
                lambda x: (x.rsID.unique().shape[0]) / total_snps).rename('counts').reset_index()
            ...
            for pheno, df in df_results_sub_grouped.groupby('phenotype'):
                ...
                plt.plot(100 * df.ablation, 100 * df.counts, label=method_dict[pheno])
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(RESULTS_DIR, 'ablation_results_plots', '{}_anchor.png'.format(code)))
            plt.show()
        except Exception as e:
            print(code, control_flag, e)
